...
install_log="${TMPDIR:-/tmp}/gitlab-register-install.log"
rm -f "$install_log"
...
echo "=== GitLab Register 安装 ==="

if [ -f /etc/debian_version ]; then
    echo "[setup] 安装系统依赖 (Debian/Ubuntu)..."
    sudo env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none apt update -qq >/dev/null 2>&1 || true
    ...
    run_optional_quiet sudo yum install -y -q \
        python3 python3-pip atk cups-libs libdrm libXcomposite libXdamage \
        libXfixes libXrandr mesa-libgbm pango cairo alsa-lib nspr nss libxshmfence
else
